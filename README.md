...
3. Make the launcher callable from anywhere by adding its directory to your `PATH` in `~/.bashrc`:
   ...
   ```
4. Make sure login shells also pick up your PATH. Login shells (e.g. `qsub -I` on compute nodes) source `~/.bash_profile` and ignore `~/.bashrc` unless told otherwise, so add a one-line bridge to `~/.bash_profile` (create the file if it doesn't exist):
   ```bash
   [ -f ~/.bashrc ] && . ~/.bashrc
   ```
   Now both interactive shells (which already source `.bashrc`) and login shells get the same PATH and env. If you'd rather not bridge, the alternative is to duplicate the `export PATH=...` line into `~/.bash_profile` directly — works, but you have to keep the two files in sync from now on.
5. Reload your shell:
   ```bash
   source ~/.bash_profile
   ```
